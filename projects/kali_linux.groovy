freeStyleJob('kali_linux') {
    ...
    steps {
        shell('docker run --rm --privileged -v $(which docker):/usr/bin/docker:ro -v $(pwd | sed \'s#/var/jenkins_home/#/var/lib/docker/jenkins/#\')/build.sh:/usr/bin/build.sh:ro -v /var/run/docker.sock:/var/run/docker.sock debian build.sh')
        ...
        shell('docker push --disable-content-trust=true jess/kalilinux:latest')
