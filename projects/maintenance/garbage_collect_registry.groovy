freeStyleJob('maintenance_garbage_collect_registry') {
    ...
    steps {
        ...
        shell('docker run --rm --disable-content-trust=false --name registry-garbage-collect -v /home/jessfraz/volumes/registry:/etc/docker/registry:ro -v /etc/ssl/certs/ca-certificates.crt:/etc/ssl/certs/ca-certificates.crt:ro registry garbage-collect /etc/docker/registry/config.yml')
