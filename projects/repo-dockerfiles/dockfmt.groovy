freeStyleJob('dockfmt') {
    ...
    steps {
        ...
        shell('if [[ "$(cat .branch)" != "latest" ]]; then docker tag r.j3ss.co/dockfmt:$(cat .branch) r.j3ss.co/dockfmt:latest; docker push --disable-content-trust=false r.j3ss.co/dockfmt:latest; fi')
        shell('if [[ "$(cat .branch)" != "latest" ]]; then docker tag jess/dockfmt:$(cat .branch) jess/dockfmt:latest; docker push --disable-content-trust=false jess/dockfmt:latest; fi')
        shell('for tag in $(git tag); do git checkout $tag; docker build  --rm --force-rm -t r.j3ss.co/dockfmt:$tag . || true; docker push --disable-content-trust=false r.j3ss.co/dockfmt:$tag || true; done')
