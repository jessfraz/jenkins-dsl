freeStyleJob('oneup') {
    ...
    steps {
        ...
        shell('for tag in $(git tag); do git checkout $tag; docker build  --rm --force-rm -t r.j3ss.co/1up:$tag . || true; docker push --disable-content-trust=false r.j3ss.co/1up:$tag || true; docker tag r.j3ss.co/1up:$tag jess/1up:$tag || true; docker push --disable-content-trust=false jess/1up:$tag || true; done')
