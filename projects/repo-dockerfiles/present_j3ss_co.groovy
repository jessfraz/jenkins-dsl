freeStyleJob('present_j3ss_co') {
    ...
    steps {
        ...
        shell('for tag in $(git tag); do git checkout $tag; docker build  --rm --force-rm -t r.j3ss.co/present:$tag . || true; docker push --disable-content-trust=false r.j3ss.co/present:$tag || true; docker tag r.j3ss.co/present:$tag jess/present:$tag || true; docker push --disable-content-trust=false jess/present:$tag || true; done')
